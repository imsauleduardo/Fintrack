...
import { LogOut, Loader2, Trash2, Edit2, Calendar } from "lucide-react";
...
export default function DashboardPage() {
    ...
    const handleFABAction = (action: "income" | "expense" | "scan" | "voice") => {
        ...
        // Configuramos valores por defecto según la acción
        if (action === "income") {
            setAiResult({ type: "income", amount: "" });
        } else if (action === "expense") {
            setAiResult({ type: "expense", amount: "" });
        }

        setShowModal(true);
    };

    // CORRECCIÓN LÍNEA 59: Usamos (prev || {}) para evitar el error con null
    ...
    return (
        <div className="min-h-screen bg-black text-white p-6 pb-32 max-w-2xl mx-auto">
            <header className="flex justify-between items-center mb-10">
                ...
                <button onClick={async () => { const { supabase } = await import('@/supabase/client'); await supabase.auth.signOut(); window.location.href = '/auth/login'; }} className="p-3 bg-white/5 rounded-2xl border border-white/10 text-gray-400 group active:scale-95 transition-all">
                            ...
                            <div key={date} className="space-y-4">
                                ...
                                <div className="space-y-3">
                                    {groupedTransactions[date].map((t: any) => {
                                        ...
                                        return (
                                            <div key={t.id} className="group relative flex items-center justify-between p-4 bg-white/5 border border-white/10 rounded-3xl hover:bg-white/[0.08] transition-all">
                                                ...
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                        <button onClick={() => { setEditingTransaction(t); setShowModal(true); }} className="p-2 bg-blue-500/10 text-blue-400 rounded-xl hover:bg-blue-500/20"><Edit2 className="w-3 h-3" /></button>
                                                        <button onClick={async () => { if (confirm("¿Borrar?")) { await deleteTransaction(t.id); loadData(); } }} className="p-2 bg-red-500/10 text-red-500 rounded-xl hover:bg-red-500/20"><Trash2 className="w-3 h-3" /></button>
                                                    </div>
                                                </div>
                                            </div>
                                        );
            ...
            >
                <div className="space-y-8">
                        ...
                        <div className="space-y-6">
                            <div className="p-6 bg-white/5 rounded-[32px] border border-white/10 flex flex-col gap-6">
                                ...
                                <ReceiptScanner
                                    ...
                                    isProcessing={isAILoading}
                                    setIsProcessing={setIsAILoading}
                                />
                    ...
                    <TransactionForm
                        ...
                        isLoading={isSaving}
                        initialData={editingTransaction || aiResult}
                        key={editingTransaction?.id || JSON.stringify(aiResult)}
                    />
                </div>
            </Modal>
